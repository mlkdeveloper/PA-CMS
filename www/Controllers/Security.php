    ...
    public function pwdPerduAction(){
        ...
        if (!empty($_POST)){
            ...
            if (empty($errors)) {
                ...
                Email::sendEmail($email, "Récupération du mot de passe","http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/recuperation-mot-de-passe?tkn=".$user[0]["token"], "Je modifie mon mot de passe", "http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/connexion");
