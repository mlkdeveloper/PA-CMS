...
function entete_table($position_entete) {
	...
	$this->SetX(10);
	$this->Cell(20,8,utf8_decode('Qté'),1,0,'C',1);

	$this->SetX(30);
	$this->Cell(90,8,utf8_decode('Désignation'),1,0,'C',1);

	$this->SetX(120);
	$this->Cell(40,8,'Prix UNIT.',1,0,'C',1);
...
{
	// Fatal error
	throw new MyException('FPDF error: '.$msg);
}
...
function AddPage($ordre, $orientation='', $size='', $rotation=0)
{
	// Start a new page
	if($this->state==3)
...
function Header($ordre)
{
	// Logo : 8 >position à gauche du document (en mm), 2 >position en haut du document, 80 >largeur de l'image en mm). La hauteur est calculée automatiquement.
	//$this->Image('images/logo/logo.png',4,4);
	// Saut de ligne 10 mm
	$this->Ln(10);
	...
	$date = date('d-m-Y', strtotime($ordre['CreatedAt'] ));

	$this->Cell(60,8,'Votre Facture du '.$date,0,1,'C',1);
	// Saut de ligne 10 mm
	$this->Ln(10);
}
